...
OUTPUT_DIR = os.path.join(PROJECT_ROOT, "output", "submission_targetwise_hybrid_ultra")

TA_BLEND_WEIGHT = 0.25
EC_BLEND_WEIGHT = 0.80
DRP_CHAIN_BLEND_WEIGHT = 0.85
...
def run():
    ...
    drp_chain_oof = blend_series(drp_no_old["oof_pred"].values, drp_base["oof_pred"].values, DRP_CHAIN_BLEND_WEIGHT)
    drp_chain_val = blend_series(drp_no_old["val_pred"].values, drp_base["val_pred"].values, DRP_CHAIN_BLEND_WEIGHT)

    drp_specialist = fit_drp_specialist_oof(train_df, val_df, specialist_features(), threshold=DRP_SPECIALIST_THRESHOLD)
    drp_oof = blend_series(drp_chain_oof, drp_specialist["oof_pred"].values, DRP_SPECIALIST_WEIGHT)
    drp_val = blend_series(drp_chain_val, drp_specialist["val_pred"].values, DRP_SPECIALIST_WEIGHT)
    ...
    estimated_public, _ = estimate_public_score(avg_spatial, 39, target_scores=target_scores)
        ...
        [
            {
                ...
                "drp_chain_blend_weight": DRP_CHAIN_BLEND_WEIGHT,
    ...
    mix_scan.to_csv(os.path.join(OUTPUT_DIR, "mix_config.csv"), index=False)

    with open(os.path.join(OUTPUT_DIR, "feature_summary.txt"), "w", encoding="utf-8") as fh:
        fh.write("[Total Alkalinity:no_quality]\n" + "\n".join(NO_QUALITY + ["Latitude", "Longitude"]) + "\n\n")
        fh.write("[Total Alkalinity:no_old]\n" + "\n".join(NO_OLD + ["Latitude", "Longitude"]) + "\n\n")
        fh.write("[Electrical Conductance:no_quality]\n" + "\n".join(NO_QUALITY + ["ta_pred"]) + "\n\n")
        fh.write("[Electrical Conductance:no_old]\n" + "\n".join(NO_OLD + ["ta_pred"]) + "\n\n")
        fh.write("[Dissolved Reactive Phosphorus:no_old]\n" + "\n".join(NO_OLD + ["Latitude", "ta_pred", "ec_pred"]) + "\n\n")
        fh.write("[Dissolved Reactive Phosphorus:base]\n" + "\n".join(COMMON_ALL + ["Latitude", "ta_pred", "ec_pred"]) + "\n\n")
    ...
    with open(os.path.join(OUTPUT_DIR, "notes.txt"), "w", encoding="utf-8") as fh:
        ...
        fh.write("EC blend = 0.20 * no_quality + 0.80 * no_old.\n")
        fh.write("DRP chain blend = 0.15 * no_old + 0.85 * base.\n")
        fh.write("DRP final = 0.65 * chain_blend + 0.35 * specialist(threshold=144).\n")
        fh.write(f"Average spatial CV = {avg_spatial:.4f}; estimated public = {estimated_public:.4f}.\n")

    print(metrics.to_string(index=False))
    print(mix_scan.to_string(index=False))
