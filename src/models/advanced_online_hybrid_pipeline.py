...
OUTPUT_DIR = os.path.join(PROJECT_ROOT, "output", "submission_advanced_online_hybrid")
RANDOM_STATE = 42
N_SPLITS = 5
...
def select_best_drp_mix(ta_r2, ec_r2, chain_oof, specialist_runs):
    rows = []
    y = chain_oof["y_true"]
    for name, run in specialist_runs.items():
        for w in [0.10, 0.15, 0.20, 0.25, 0.30, 0.35]:
            pred = (1.0 - w) * chain_oof["pred"] + w * run["oof_pred"].values
            ...
            est_public, _ = estimate_public_score(avg_spatial, 46, target_scores=target_scores)
            rows.append({
                "variant": name,
                "weight": w,
                "drp_r2": drp_r2,
    ...
    scan = pd.DataFrame(rows).sort_values(["estimated_public", "drp_r2"], ascending=False).reset_index(drop=True)
...
def run():
    ...
    chain_mask = train_df["api_optical_complete"] == 1
    chain_oof = {
        "y_true": train_df.loc[chain_mask, "Dissolved Reactive Phosphorus"].reset_index(drop=True),
        "pred": drp_chain["oof_pred"].reset_index(drop=True).values,
    }
    scan, best = select_best_drp_mix(float(ta["CV_R2"]), float(ec["CV_R2"]), chain_oof, specialist_runs)
    scan.to_csv(os.path.join(OUTPUT_DIR, "drp_mix_scan.csv"), index=False)

    best_run = specialist_runs[best["variant"]]
    w = float(best["weight"])
    drp_val = (1.0 - w) * drp_chain["val_pred"].values + w * best_run["val_pred"].values
    ...
    metrics = pd.DataFrame(
        [
            ...
            {"Parameter": "Dissolved Reactive Phosphorus", "CV_R2": best["drp_r2"], "CV_RMSE": float(np.sqrt(mean_squared_error(chain_oof["y_true"], (1.0 - w) * chain_oof["pred"] + w * best_run["oof_pred"].values))), "rows_used": best_run["rows_used"]},
        ]
    )
    ...
    with open(os.path.join(OUTPUT_DIR, "feature_summary.txt"), "w", encoding="utf-8") as fh:
        ...
        fh.write("threshold sweep over 96 and 128 with online-calibrated blend search\n")
    ...
    with open(os.path.join(OUTPUT_DIR, "notes.txt"), "w", encoding="utf-8") as fh:
        fh.write(f"Best DRP mix: {best['variant']} weight={best['weight']:.2f}\\n")
        fh.write(f"Estimated public: {best['estimated_public']:.4f}\\n")
        fh.write("TA/EC remain advanced_stacking target-chain stackers.\\n")
